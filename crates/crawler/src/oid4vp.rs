...
pub async fn present(
    ...
    let response = http
        .post(response_uri)
        .form(&[
            ("vp_token", vp_token.as_str()),
            ("presentation_submission", presentation_submission.as_str()),
        ])
        .send()
        .await
        .map_err(|source| Oid4VpError::Transport {
    ...
    let body: Value = response
        .json()
        .await
        .map_err(|source| Oid4VpError::MalformedResponse {
            uri: response_uri.to_string(),
            source,
        })?;
    ...
    body.get("access_token")
        ...
        .map(str::to_string)
        .ok_or_else(|| Oid4VpError::MissingAccessToken {
            uri: response_uri.to_string(),
        })
}
...
mod tests {
    ...
    use dcp_core::{
        DcpKeyPair, DidDocument, decode_jws_unverified, find_verifying_key, verify_jws_signature,
    };
    use serde_json::{Value, json};
    ...
    fn assert_signature_verifies(token: &str, key_pair: &DcpKeyPair) {
        let doc: DidDocument =
            serde_json::from_value(key_pair.did_document(&[])).expect("valid did document");
        let verifying_key = find_verifying_key(&doc, &key_pair.own_key_id)
            .expect("key pair's own kid is in its own did document");
        verify_jws_signature(token, &verifying_key)
            .expect("signature must verify against the key pair's own public key");
    }
    ...
    fn build_vp_token_produces_a_three_segment_jws_with_the_documented_claims() {
        ...
        assert_eq!(
            token.split('.').count(),
            3,
            "a compact JWS has exactly 3 dot-separated segments"
        );
        ...
        assert_eq!(
            exp,
            iat + 300,
            "exp must be iat + 300s, matching DCP's own T1/T2 lifetime"
        );
    ...
    fn build_presentation_submission_has_the_documented_fixed_shape() {
        ...
        assert!(
            ...
            "submission needs its own fresh id"
        );
        assert_eq!(
            submission["definition_id"],
            json!("federated-catalog-access")
        );

        let descriptor_map = submission["descriptor_map"]
            .as_array()
            .expect("descriptor_map array");
        assert_eq!(
            descriptor_map.len(),
            1,
            "this project expects exactly one credential type per participant"
        );
        assert_eq!(
            descriptor_map[0]["id"],
            json!("federated-catalog-access-credential")
        );
    ...
    async fn present_returns_the_access_token_on_a_real_200_response() {
        ...
        let app = Router::new().route(
            ...
            post(|| {
                respond_json(
                    ...
                    json!({"access_token": "mock-access-token-123"}),
                )
            }),
        );
        spawn(listener, app);
        ...
        let token = present(&http, &key_pair, CREDENTIAL_JWS, &response_uri)
            .await
    ...
    async fn present_returns_a_distinct_error_for_a_non_2xx_response() {
        ...
        assert!(
            matches!(err, Oid4VpError::NonSuccessStatus { .. }),
    ...
    async fn present_returns_a_distinct_error_for_a_response_missing_access_token() {
        ...
        assert!(
            matches!(err, Oid4VpError::MissingAccessToken { .. }),
            "unexpected error variant: {err:?}"
        );
    }
    ...
    async fn present_returns_a_distinct_error_for_a_transport_failure() {
        ...
        let err = present(
            ...
            &key_pair,
            CREDENTIAL_JWS,
            "http://127.0.0.1:0/oid4vp/response",
        )
        .await
        .expect_err("should fail when the response_uri is unreachable");

        assert!(
            matches!(err, Oid4VpError::Transport { .. }),
            "unexpected error variant: {err:?}"
        );
    }
}
