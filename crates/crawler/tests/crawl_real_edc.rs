...
const PARTICIPANTS_TOML: &str = include_str!("../../../compliance/crawler-edc-fixture/participants.toml");
...
async fn crawls_three_real_edc_instances_and_aggregates_all_seeded_datasets() {
    let config = ParticipantsConfig::parse(PARTICIPANTS_TOML, "compliance/crawler-edc-fixture/participants.toml")
        .expect("participants.toml should parse and validate");
    assert_eq!(config.participants.len(), 3, "expected exactly the three real-EDC participants");
    ...
    assert_eq!(summary.succeeded, 3, "expected all three real-EDC instances to be crawled successfully");

    let catalogs = cache.query(CatalogQuery::all()).await.expect("querying the cache should not fail");
    assert_eq!(catalogs.len(), 3, "expected one cached Catalog per real-EDC instance, got {}", catalogs.len());

    let observed_ids: BTreeSet<String> =
        catalogs.iter().flat_map(|catalog| catalog.datasets.iter().map(|dataset| dataset.id.clone())).collect();

    let expected_ids: BTreeSet<String> = EXPECTED_DATASET_IDS.iter().map(|s| s.to_string()).collect();
